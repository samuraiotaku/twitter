...
class APIManager: SessionManager {
    ...
    static let consumerKey = "rtqdOmOouZG9BT3AWZiWEhpcm"
    static let consumerSecret =  "HKklfRzgC1calIjVNSJMjZBHzPnHr9ycZ2YRfemm9v7lH3bCLP"
    ...
    func favorite(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        ...
        request(URL(string: urlString)!, method: .post, parameters: parameters as Parameters, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
    ...
    func retweet(with tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        ...
        let idString = "\(tweet.id!)"
        let parameters = ["id": tweet.id]
        let url = urlString + idString + ".json"
        print(url)
        request(url, method: .post, parameters: parameters as Parameters, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
            if response.result.isSuccess,
                let tweetDictionary = response.result.value as? [String: Any] {
                let tweet = Tweet(dictionary: tweetDictionary)
                completion(tweet, nil)
    ...
    func untweet(with tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
